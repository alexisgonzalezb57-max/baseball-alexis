...
if ($equipo < 1) {
    ...
    // Consulta modificada para incluir solo equipos clasificados
    if ($valorval > 0) {
        $covl = "SELECT rs.*, MAX(rs.a) as max_asistencias
                ...
                JOIN equipo_estados ee ON rs.id_team = ee.id_tab AND ee.estado = 'C'
    ...
    if ($asnm >= 1) {
        ...
        for ($jg=1; $jg <= $asnm ; $jg++) { 
            ...
            if ($valorval <= 0) {
                ...
                // Cuando $valorval > 0, mostrar todos los jugadores sin límite por equipo
                if ($id_team !== $equipoActual) {
                    $equipoActual = $id_team;
                    $contadorEquipos++;
                }
    ...
    // Código para un equipo específico (similar al original)
    // ... (código existente para manejar un equipo específico)
}
