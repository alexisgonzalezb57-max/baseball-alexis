...
<head>
    ...
    <title>Editar Homenaje - Sistema de Homenajes</title>
...
<body>
    ...
    <div class="container" style="margin-top: -750px;">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">✏️ Editar homenaje</h4>
                <h6 class="card-subtitle mb-4 text-muted">Modifique los datos que desee actualizar</h6>
                
                <form method="POST" action="update.php">
                    <input type="hidden" name="id" value="<?php echo $homenaje['id']; ?>">
                            ...
                            <div class="col-md-4">
                                <select class="form-control" id="tempo" name="temporada" required>
                                    ...
                                    if (!empty($homenaje['categoria'])) {
                                        ...
                                        $query_tempo = "SELECT * FROM temporadas WHERE categoria = '$cat_actual'";
                                        $result_tempo = mysqli_query($con, $query_tempo);
                                        
                                        while($tempo = mysqli_fetch_array($result_tempo)) {
                                            $selected = ($tempo['id'] == $homenaje['id_temp']) ? 'selected' : '';
                                            echo "<option value='{$tempo['id']}' $selected>{$tempo['nombre']}</option>";
                                        }
                                    }
                                    ?>
                                </select>
                    ...
                    <div class="form-group row mt-4">
                        <div class="col-md-12 text-center">
                            <button type="submit" class="btn btn-success btn-action mr-3">💾 Actualizar</button>
                            <a href="homenaje.php" class="btn btn-secondary btn-action">↩ Cancelar</a>
                        </div>
                    </div>
                </form>
            </div>
