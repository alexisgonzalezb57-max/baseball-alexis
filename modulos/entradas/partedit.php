...
$id_juego = filter_var($_REQUEST['id_juego'] ?? 0, FILTER_SANITIZE_NUMBER_INT);

// Consulta preparada para obtener datos del juego
$teamss = "SELECT * FROM juegos WHERE id_juego = ?";
$stmt = mysqli_prepare($con, $teamss);
...
if (!$data) {
    die("No se encontraron datos para el juego especificado.");
}
...
// Consulta preparada para obtener información de clasificación
...
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
$redt = mysqli_fetch_array($result);
...
<head>
    ...
    <title>Editar Partido - Sistema Baseball</title>
    ...
    <main class="main-content">
        <div class="container">
            <div class="content-container">
                <h1 class="page-title">Editar Partido</h1>
                <h4 class="card-title"><?php echo htmlspecialchars($redt['name_team']); ?> - Partido N° <?php echo $nj; ?></h4>
                
                <form method="POST" action="partupdate.php">
                    <input type="hidden" name="id" value="<?php echo $id_juego; ?>">
                    <input type="hidden" name="team_one" value="<?php echo $one; ?>">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-2">
                            <input type="number" min="1" value="<?php echo $data['jj']; ?>" required class="form-control" name="jj">
                        </div>
                        <label class="col-sm-3 col-form-label text-end">Fecha</label>
                        <div class="col-sm-4">
                            <input type="date" required class="form-control" value="<?php echo $data['fech_part']; ?>" name="fech_part">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-5">
                            <input type="text" class="form-control-plaintext" value="<?php echo htmlspecialchars($redt['name_team']); ?>" readonly>
                        </div>
                        <label class="col-sm-1 col-form-label">CA</label>
                        <div class="col-sm-2">
                            <input type="number" class="form-control" id="ca" min="0" value="<?php echo $data['ca']; ?>" name="ca">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-5">
                            <?php if ($two > 0): ?>
                                <select class="form-select" required name="team_twob" disabled>
                                    <?php 
                                    // Consulta preparada para obtener equipos disponibles
                                    $team = "SELECT * FROM tab_clasf WHERE id_temp = ? AND id_tab != ?";
                                    $stmt_teams = mysqli_prepare($con, $team);
                                    mysqli_stmt_bind_param($stmt_teams, "ii", $temp, $id_tab);
                                    mysqli_stmt_execute($stmt_teams);
                                    $obte = mysqli_stmt_get_result($stmt_teams);
                                    
                                    while ($dte = mysqli_fetch_array($obte)) {
                                        $selected = $dte['id_team'] == $two ? "selected" : "";
                                        echo '<option ' . $selected . ' value="' . $dte['id_team'] . '">' . htmlspecialchars($dte['name_team']) . '</option>';
                                    }
                                    ?>
                                </select>
                            <?php else: ?>
                            ...
                            <?php endif; ?>
                            <input type="hidden" name="team_two" value="<?php echo $two; ?>">
                        </div>
                        <label class="col-sm-1 col-form-label">CE</label>
                        <div class="col-sm-2">
                            <input type="number" class="form-control" id="ce" min="0" value="<?php echo $data['ce']; ?>" name="ce">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-5">
                            <div class="resultado-box" id="estado-container">
                                <input type="hidden" name="estado" id="estado-input" value="<?php echo htmlspecialchars($data['estado']); ?>">
                                <span id="estado-texto" class="
                                    <?php 
                                    if ($data['estado'] === 'Ganando') echo 'estado-ganando';
                                    elseif ($data['estado'] === 'Perdido') echo 'estado-perdido';
                                    else echo 'estado-empatado';
                                    ?>
                                "><?php echo htmlspecialchars($data['estado']); ?></span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-6">
                            <div class="radio-group">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="val" id="validoSi" value="1" <?php echo !empty($data['valido']) ? 'checked' : ''; ?>>
                                    <label class="form-check-label" for="validoSi">Sí</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="val" id="validoNo" value="0" <?php echo empty($data['valido']) ? 'checked' : ''; ?>>
